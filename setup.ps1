...
function Download-Config($Filename, $Dest) {
    if ($env:SETUP_REMOTE_BASE) {
        try {
            Invoke-Pending "下载 $Filename" {
                Invoke-WebRequest -Uri "$env:SETUP_REMOTE_BASE/$Filename" -OutFile $Dest
            }
            return $true
        } catch {
            Write-Error "下载 $Filename 失败"
        }
    }
    return $false
}
...
if (-not $copied) {
    ...
    if (-not $env:SETUP_REMOTE_BASE) {
        Write-Warn "提示: 设置 `SETUP_REMOTE_BASE` 环境变量指向你的仓库"
    }
}
Write-Host ""
